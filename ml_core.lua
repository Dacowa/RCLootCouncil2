...
function RCLootCouncilML:ItemsInBagsLowTradeTimeRemainingReminder()
	...
	if #entriesToRemind > 0 then
		...
		for _, v in ipairs(entriesToRemind) do
			...
			v.Item.args.recipient and addon:GetClassIconAndColoredName(v.Item.args.recipient) or L["Unawarded"],
...
--	history_table["relicRoll"]		= relicRoll																			-- New in v2.5+ - Removed v2.9
	...
	-- or Personal Loot/Bonus Roll/non-tradeable loot that was never actually rolled through the priority system.
	if not (self.lootTable[session] and self.lootTable[session].isFFA)
		and responseID ~= "PL" and responseID ~= "PL_REJECT" and responseID ~= "BONUSROLL" then
