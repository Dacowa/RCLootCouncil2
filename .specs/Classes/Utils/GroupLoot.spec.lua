...
describe("#GroupLoot", function()
	...
	describe("#Status", function()
		...
		it("statusToDescription should return a table", function()
			...
			local status, target = "1", "111111111"
			assert.equal(9, #GroupLoot:StatusToDescription(status, target)) -- 1 -> 0 0000 0001 after conversion
			assert.equal(WrapTextInColorCode("Received data from ML", "FF00FF00"), GroupLoot:StatusToDescription(status, target)[1])
			assert.equal(WrapTextInColorCode("ML has disabled autoGroupLoot", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[2])
			assert.equal(WrapTextInColorCode("RCLootCouncil doesn't handle loot", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[3])
			assert.equal(WrapTextInColorCode("Group has no master looter", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[4])
			assert.equal(WrapTextInColorCode("Player is not master looter", "FFFF0000"),	GroupLoot:StatusToDescription(status, target)[5])
			assert.equal(WrapTextInColorCode("Not in a group", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[6])
			assert.equal("Guild Groups Only setting", GroupLoot:StatusToDescription(status, target)[7])
			assert.equal(WrapTextInColorCode("Guild group settings", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[8])
			assert.equal(WrapTextInColorCode("RCLootCouncil disabled", "FFFF0000"), GroupLoot:StatusToDescription(status, target)[9])
			status, target = "101100000", "011011011"
			...
			assert.equal("|cFF00FF00ML has enabled autoGroupLoot|r", GroupLoot:StatusToDescription(2, 2)[2])
			assert.equal("|cFFFF0000ML has enabled autoGroupLoot|r", GroupLoot:StatusToDescription(2, 1)[2])
			assert.equal("|cFFFF0000ML has disabled autoGroupLoot|r", GroupLoot:StatusToDescription(0, 2)[2])
		end)
